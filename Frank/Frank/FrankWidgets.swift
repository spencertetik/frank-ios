...
struct FrankWidgetProvider: TimelineProvider {
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<FrankWidgetEntry>) -> ()) {
        // TODO: In a real implementation, fetch data from shared container or UserDefaults
        ...
        let entry = FrankWidgetEntry(
            ...
            currentTask: ["Monitoring systems", "Processing requests", "Analyzing data", "Optimizing workflows"].randomElement() ?? "Standby",
            nextEvent: CalendarWidgetEvent(title: "Daily Standup", startTime: currentDate.addingTimeInterval(Double.random(in: 300...7200))),
            upcomingEvents: generateSampleEvents(),
            messagesCount: Int.random(in: 0...50),
            lastMessage: ["All systems operational", "Ready for commands", "Processing complete", "Monitoring active"].randomElement() ?? "Ready"
        )
    ...
    private func generateSampleEvents() -> [CalendarWidgetEvent] {
        let baseDate = Date()
        return [
            CalendarWidgetEvent(title: "Daily Standup", startTime: baseDate.addingTimeInterval(1800)),
            CalendarWidgetEvent(title: "Design Review", startTime: baseDate.addingTimeInterval(3600)),
            CalendarWidgetEvent(title: "Client Call", startTime: baseDate.addingTimeInterval(7200))
        ]
    }
}
